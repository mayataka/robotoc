...
void BenchmarkWithoutContacts() {
  ...
  joint_cost->set_a_weight(Eigen::VectorXd::Constant(robot.dimv(), 0.001));
  joint_cost->set_u_weight(Eigen::VectorXd::Constant(robot.dimv(), 0.001));
  ...
  parnmpc_benchmarker.testConvergence(t, q, v, 100, false);
  auto datas = constraints->createConstraintsData(robot);
