...
int main () {
  ...
  const double dtau_near_zero = (1 + std::abs(Eigen::VectorXd::Random(1).coeff(0))) * std::numeric_limits<double>::epsilon();
  std::cout << "double eps = " << std::numeric_limits<double>::epsilon() << std::endl;
  std::cout << "dtau_near_zero = " << dtau_near_zero << std::endl;
  const double switching_time = 0.25 + dtau_near_zero;
  ...
  ocp_benchmarker.testConvergence(t, q, v, 5, false);
  const double t_next = t + 0.0025 + dtau_near_zero;
  ocp_benchmarker.testConvergence(t_next, q, v, 5, false);
  // ocp_benchmarker.testCPUTime(t, q, v, 1000);
  ocp_benchmarker.getSolverHandle()->printSolution("f");
