...
class BipedWalkFootStepPlannerTest : public ::testing::Test {
protected:
  virtual void SetUp() {
  }
...
TEST_F(BipedWalkFootStepPlannerTest, enableDoubleSupportPhase) {
  const double dt = 0.05;
  auto robot = testhelper::CreateHumanoidRobot(dt);
  auto planner = std::make_shared<BipedWalkFootStepPlanner>(robot);
  ...
  const bool enable_double_support_phase = (stance_time > 0.0);
  planner->setGaitPattern(step_length, step_yaw, enable_double_support_phase);
...
TEST_F(BipedWalkFootStepPlannerTest, disableDoubleSupportPhase) {
  const double dt = 0.05;
  auto robot = testhelper::CreateHumanoidRobot(dt);
  auto planner = std::make_shared<BipedWalkFootStepPlanner>(robot);
  const Eigen::Vector3d vcom_cmd = Eigen::Vector3d::Random();
