...
class PaceFootStepPlannerTest : public ::testing::Test {
protected:
  virtual void SetUp() {
  }
...
TEST_F(PaceFootStepPlannerTest, enableStancePhase) {
  const double dt = 0.05;
  auto robot = testhelper::CreateQuadrupedalRobot(dt);
  auto planner = std::make_shared<PaceFootStepPlanner>(robot);
...
TEST_F(PaceFootStepPlannerTest, disableStancePhase) {
  const double dt = 0.05;
  auto robot = testhelper::CreateQuadrupedalRobot(dt);
  auto planner = std::make_shared<PaceFootStepPlanner>(robot);
  const Eigen::Vector3d vcom_cmd = Eigen::Vector3d::Random();
