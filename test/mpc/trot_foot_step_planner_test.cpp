...
class TrotFootStepPlannerTest : public ::testing::Test {
protected:
  virtual void SetUp() {
  }
...
TEST_F(TrotFootStepPlannerTest, enableStancePhase) {
  const double dt = 0.05;
  auto robot = testhelper::CreateQuadrupedalRobot(dt);
  auto planner = std::make_shared<TrotFootStepPlanner>(robot);
...
TEST_F(TrotFootStepPlannerTest, disableStancePhase) {
  const double dt = 0.05;
  auto robot = testhelper::CreateQuadrupedalRobot(dt);
  auto planner = std::make_shared<TrotFootStepPlanner>(robot);
  const Eigen::Vector3d vcom_cmd = Eigen::Vector3d::Random();
