...
class CrawlFootStepPlannerTest : public ::testing::Test {
protected:
  virtual void SetUp() {
  }
...
TEST_F(CrawlFootStepPlannerTest, enableStancePhase) {
  const double dt = 0.05;
  auto robot = testhelper::CreateQuadrupedalRobot(dt);
  auto planner = std::make_shared<CrawlFootStepPlanner>(robot);
  ...
  const bool enable_stance_phase = (stance_time > 0.0);
  planner->setGaitPattern(step_length, step_yaw, enable_stance_phase);
...
TEST_F(CrawlFootStepPlannerTest, disableStancePhase) {
  const double dt = 0.05;
  auto robot = testhelper::CreateQuadrupedalRobot(dt);
  auto planner = std::make_shared<CrawlFootStepPlanner>(robot);
  const Eigen::Vector3d vcom_cmd = Eigen::Vector3d::Random();
