...
class MultiModeTaskSpace6DCostTest : public ::testing::Test {
protected:
  virtual void SetUp() {
...
void MultiModeTaskSpace6DCostTest::testStageCost(Robot& robot, const int frame_id) const {
  ...
  auto cost = std::make_shared<MultiModeTaskSpace6DCost>(robot, frame_id);
  CostFunctionData data(robot);
  EXPECT_TRUE(cost->useKinematics());
  cost->set_x6d_weight(x6d_weight.tail(3), x6d_weight.head(3), 0);
  cost->set_x6df_weight(x6df_weight.tail(3), x6df_weight.head(3));
  cost->set_x6di_weight(x6di_weight.tail(3), x6di_weight.head(3));
  cost->set_x6d_ref(position_ref, rotation_ref, 0);
  cost->set_x6d_ref(position_ref, rotation_ref, {0, 1, 2, 3});
...
void MultiModeTaskSpace6DCostTest::testTerminalCost(Robot& robot, const int frame_id) const {
  const int dimv = robot.dimv();
  auto kkt_mat = SplitKKTMatrix::Random(robot);
  auto kkt_res = SplitKKTResidual::Random(robot);
  auto kkt_mat_ref = kkt_mat;
...
void MultiModeTaskSpace6DCostTest::testImpulseCost(Robot& robot, const int frame_id) const {
  const int dimv = robot.dimv();
