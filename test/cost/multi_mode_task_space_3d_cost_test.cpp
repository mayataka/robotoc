...
class MultiModeTaskSpace3DCostTest : public ::testing::Test {
protected:
  virtual void SetUp() {
...
void MultiModeTaskSpace3DCostTest::testStageCost(Robot& robot, const int frame_id) const {
  ...
  auto cost = std::make_shared<MultiModeTaskSpace3DCost >(robot, frame_id);
  CostFunctionData data(robot);
  EXPECT_TRUE(cost->useKinematics());
  cost->set_weight(x3d_weight, 0);
  cost->set_weight(x3d_weight, {0, 1, 2, 3});
  cost->set_weight_terminal(x3df_weight);
  cost->set_weight_impulse(x3di_weight);
  cost->set_ref(q_ref, 0);
...
void MultiModeTaskSpace3DCostTest::testTerminalCost(Robot& robot, const int frame_id) const {
  const int dimv = robot.dimv();
  auto kkt_mat = SplitKKTMatrix::Random(robot);
  auto kkt_res = SplitKKTResidual::Random(robot);
  auto kkt_mat_ref = kkt_mat;
...
void MultiModeTaskSpace3DCostTest::testImpulseCost(Robot& robot, const int frame_id) const {
  const int dimv = robot.dimv();
...
TEST_F(MultiModeTaskSpace3DCostTest, fixedBase) {
...
TEST_F(MultiModeTaskSpace3DCostTest, floatingBase) {
