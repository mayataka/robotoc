...
TEST_F(ContactDynamicsDataTest, fixedBase) {
  ...
  EXPECT_EQ(data.dCda().rows(), 0);
  EXPECT_EQ(data.dCda().cols(), dimv);
  EXPECT_EQ(data.dIDCdqv().rows(), dimv);
  ...
  EXPECT_EQ(data.dCdq().rows(), 0);
  EXPECT_EQ(data.dCdq().cols(), dimv);
  EXPECT_EQ(data.dCdv().rows(), 0);
  EXPECT_EQ(data.dCdv().cols(), dimv);
  EXPECT_EQ(data.MJtJinv().rows(), dimv);
  EXPECT_EQ(data.MJtJinv().cols(), dimv);
  EXPECT_EQ(data.MJtJinv_dIDCdqv().rows(), dimv);
  EXPECT_EQ(data.MJtJinv_dIDCdqv().cols(), dimx);
  EXPECT_EQ(data.Qafqv().rows(), dimv);
  EXPECT_EQ(data.Qafqv().cols(), dimx);
  EXPECT_EQ(data.Qafu_full().rows(), dimv);
  EXPECT_EQ(data.Qafu_full().cols(), dimv);
  EXPECT_EQ(data.Qafu_passive().rows(), dimv);
  EXPECT_EQ(data.Qafu_passive().cols(), 0);
  EXPECT_EQ(data.Qafu().rows(), dimv);
  EXPECT_EQ(data.Qafu().cols(), dimu);
  EXPECT_EQ(data.IDC().size(), dimv);
  EXPECT_EQ(data.ID_full().size(), dimv);
  EXPECT_EQ(data.ID_passive().size(), 0);
  EXPECT_EQ(data.ID().size(), dimu);
  EXPECT_EQ(data.C().size(), 0);
  EXPECT_EQ(data.MJtJinv_IDC().size(), dimv);
  EXPECT_EQ(data.laf().size(), dimv);
  EXPECT_EQ(data.la().size(), dimv);
  EXPECT_EQ(data.lf().size(), 0);
  ...
  const Eigen::MatrixXd Qafu_full_ref = Eigen::MatrixXd::Random(dimv+dimf, dimu);
  ...
  EXPECT_TRUE(data.Qafu_passive().isApprox(Qafu_full_ref.leftCols(0)));
  ...
  EXPECT_TRUE(data.ID_passive().isApprox(IDC_ref.head(0)));
  EXPECT_TRUE(data.ID().isApprox(IDC_ref.segment(0, dimu)));
...
TEST_F(ContactDynamicsDataTest, floatingBase) {
  ...
  EXPECT_EQ(data.Qafu_full().cols(), dimv);
  EXPECT_EQ(data.Qafu_passive().rows(), dimv);
  EXPECT_EQ(data.Qafu_passive().cols(), 6);
  EXPECT_EQ(data.Qafu().rows(), dimv);
  EXPECT_EQ(data.Qafu().cols(), dimu);
  EXPECT_EQ(data.IDC().size(), dimv);
  EXPECT_EQ(data.ID_full().size(), dimv);
  EXPECT_EQ(data.ID_passive().size(), 6);
  EXPECT_EQ(data.ID().size(), dimu);
  EXPECT_EQ(data.C().size(), 0);
  EXPECT_EQ(data.MJtJinv_IDC().size(), dimv);
  EXPECT_EQ(data.laf().size(), dimv);
  ...
  EXPECT_TRUE(data.Qafqv().isApprox(Qafqv_ref));
  EXPECT_TRUE(data.Qafu_full().isApprox(Qafu_full_ref));
  EXPECT_TRUE(data.Qafu_passive().isApprox(Qafu_full_ref.leftCols(6)));
  EXPECT_TRUE(data.Qafu().isApprox(Qafu_full_ref.rightCols(dimu)));
  EXPECT_TRUE(data.IDC().isApprox(IDC_ref));
  EXPECT_TRUE(data.ID_full().isApprox(IDC_ref.head(dimv)));
  EXPECT_TRUE(data.ID_passive().isApprox(IDC_ref.head(6)));
  EXPECT_TRUE(data.ID().isApprox(IDC_ref.segment(6, dimu)));
  EXPECT_TRUE(data.C().isApprox(IDC_ref.segment(dimv, dimf)));
  EXPECT_TRUE(data.MJtJinv_IDC().isApprox(MJtJinv_IDC_ref));
  EXPECT_TRUE(data.laf().isApprox(laf_ref));
  EXPECT_TRUE(data.la().isApprox(laf_ref.head(dimv)));
