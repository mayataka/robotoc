...
TEST_F(RiccatiMatrixInverterTest, fixed_base_without_contacts2) {
  const int dimv = fixed_base_robot_.dimv();
  const int dimf = fixed_base_robot_.max_dimf();
  const int dimc = dimf + fixed_base_robot_.dim_passive();
  ASSERT_TRUE(dimf == 0);
  ASSERT_TRUE(dimc == 0);
...
TEST_F(RiccatiMatrixInverterTest, fixed_base_with_contacts3) {
  const int contact_frame = 18;
  const std::vector<int> contact_frames = {contact_frame};
  fixed_base_robot_ = Robot(fixed_base_urdf_, contact_frames, 0, 0);
  const std::vector<bool> active_contacts = {true};
  fixed_base_robot_.setContactStatus(active_contacts);
  const int dimv = fixed_base_robot_.dimv();
  const int max_dimf = fixed_base_robot_.max_dimf();
  const int dimf = fixed_base_robot_.dimf();
  const int max_dimc = max_dimf + fixed_base_robot_.dim_passive();
  const int dimc = dimf + fixed_base_robot_.dim_passive();
  const int dim_passive = fixed_base_robot_.dim_passive();
  ...
  M.block(dimv, dimv+dimf, dimf, dim_passive) = Cf.leftCols(dimf).transpose();
  M.triangularView<Eigen::StrictlyLower>() 
      = M.transpose().triangularView<Eigen::StrictlyLower>();
  const Eigen::MatrixXd Minv = M.inverse();
  RiccatiMatrixInverter inverter(fixed_base_robot_);
  ...
  EXPECT_TRUE(Kaq.isApprox(Kaq_ref));
  EXPECT_TRUE(Kav.isApprox(Kav_ref));
  EXPECT_TRUE(Kfq.topRows(dimf).isApprox(Kfq_ref));
  EXPECT_TRUE(Kfv.topRows(dimf).isApprox(Kfv_ref));
  EXPECT_TRUE(Kmuq.topRows(dimc).isApprox(Kmuq_ref));
  EXPECT_TRUE(Kmuv.topRows(dimc).isApprox(Kmuv_ref));
  EXPECT_TRUE(ka.isApprox(ka_ref));
  EXPECT_TRUE(kf.head(dimf).isApprox(kf_ref));
  EXPECT_TRUE(kmu.head(dimc).isApprox(kmu_ref));
  std::cout << "Kaq error:" << std::endl;
  std::cout << Kaq - Kaq_ref << std::endl;
  std::cout << std::endl;
  std::cout << "Kav error:" << std::endl;
  ...
  std::cout << std::endl;
  std::cout << "Kfq error:" << std::endl;
  std::cout << Kfq.topRows(dimf) - Kfq_ref << std::endl;
  std::cout << std::endl;
  std::cout << "Kfv error:" << std::endl;
  std::cout << Kfv.topRows(dimf) - Kfv_ref << std::endl;
  std::cout << std::endl;
  std::cout << "Kmuq error:" << std::endl;
  std::cout << Kmuq.topRows(dimc) - Kmuq_ref << std::endl;
  std::cout << std::endl;
  std::cout << "Kmuv error:" << std::endl;
  std::cout << Kmuv.topRows(dimc) - Kmuv_ref << std::endl;
  std::cout << std::endl;
  std::cout << "ka error:" << std::endl;
  std::cout << ka - ka_ref << std::endl;
  std::cout << std::endl;
  std::cout << "kf error:" << std::endl;
  std::cout << kf.head(dimf) - kf_ref << std::endl;
  std::cout << std::endl;
  std::cout << "kmu error:" << std::endl;
  std::cout << kmu.head(dimc) - kmu_ref << std::endl;
  std::cout << std::endl;
}
...
TEST_F(RiccatiMatrixInverterTest, move) {
  const std::vector<int> contact_frames = {14, 24, 34, 44};
  floating_base_robot_ = Robot(floating_base_urdf_, contact_frames, 0, 0);
  std::vector<bool> active_contacts;
  std::random_device rnd;
  ...
  inverter.setContactStatus(floating_base_robot_);
  inverter.precompute(Qaf, Qff);
  RiccatiMatrixInverter inverter_ref = std::move(inverter);
  inverter_ref.invert(Qqa, Qva, Qaa, Qqf, Qvf, Cq, Cv, Ca, Cf, la, lf, C_res, 
                      Kaq, Kav, Kfq, Kfv, Kmuq, Kmuv, ka, kf, kmu);
  const Eigen::MatrixXd Kaq_ref = - Minv.block(0, 0, dimv, dimv) * Qqa.transpose()
                                  - Minv.block(0, dimv, dimv, dimf) * Qqf.leftCols(dimf).transpose()
                                  - Minv.block(0, dimv+dimf, dimv, dimc) * Cq.topRows(dimc);
