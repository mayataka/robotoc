...
protected:
  ...
  std::unique_ptr<manipulator::CostFunction> cost_ref_;
  std::unique_ptr<manipulator::Constraints > constraints_ref_;
...
TEST_F(FixedBaseSplitOCPTest, withoutContacts) {
  ...
  robot_.subtractConfiguration(q_, q_next_, q_res);
  ...
  cost_ref_->lu(robot_, t_, dtau_, u_, lu);
  ...
  cost_ref_->lq(robot_, t_, dtau_, q_, v_, a_, lq);
  cost_ref_->lv(robot_, t_, dtau_, q_, v_, a_, lv);
  cost_ref_->la(robot_, t_, dtau_, q_, v_, a_, la);
  ...
  const double cost_ref 
      = cost_ref_->l(robot_, t_, dtau_, q_tmp, v_tmp, a_tmp, u_tmp, f_) 
        + joint_space_constraints_ref_.costSlackBarrier(max_primal_step_size);
  ...
  const double violation_ref
      = q_res_tmp.lpNorm<1>() + v_res_tmp.lpNorm<1>() 
          ...
          + joint_space_constraints_ref_.residualL1Nrom(dtau_, q_tmp, v_tmp, a_tmp, u_tmp);
...
TEST_F(FixedBaseSplitOCPTest, withoutActiveContacts) {
  ...
  ASSERT_TRUE(ocp.isFeasible(robot_, q_, v_, a_, u_));
  ocp.initConstraints(robot_, time_step_, dtau_, q_, v_, a_, u_);
  ocp.linearizeOCP(robot_, t_, dtau_, lmd_, gmm_, q_, v_, a_, u_, f_, mu_, 
  ...
  ocp.forwardRiccatiRecursion(dtau_, dq, dv, dq_next, dv_next);
  ocp.computeCondensedDirection(dtau_, dq, dv);
  const double primal_step_size = ocp.maxPrimalStepSize();
  const double dual_step_size = ocp.maxPrimalStepSize();
  const std::pair<double, double> cost_and_violation 
      = ocp.costAndConstraintsViolation(robot_, primal_step_size, t_, 
                                        dtau_, q_, v_, a_, u_, f_, q_next_, 
                                        v_next_, dq, dv, dq_next, dv_next);
  ...
  const double KKT_error 
      = ocp.squaredKKTErrorNorm(robot_, t_, dtau_, lmd_, gmm_, q_, v_, a_, u_,
                                beta, f_, mu_, lmd_next_, gmm_next_, q_next_, 
                                v_next_);
  ...
  constraints_.reset();
  constraints_ = std::make_unique<manipulator::Constraints>(robot_);
  SplitOCP ocp_ref(robot_, cost_, constraints_);
  ASSERT_TRUE(ocp_ref.isFeasible(robot_, q_, v_, a_, u_));
  ocp_ref.initConstraints(robot_, time_step_, dtau_, q_, v_, a_, u_);
  ocp_ref.linearizeOCP(robot_, t_, dtau_, lmd_, gmm_, q_, v_, a_, u_, f_, mu_,
                       lmd_next_, gmm_next_, q_next_, v_next_);
  Eigen::MatrixXd Pqq_ref = Eigen::MatrixXd::Zero(dimv_, dimv_);
  Eigen::MatrixXd Pqv_ref = Eigen::MatrixXd::Zero(dimv_, dimv_);
  Eigen::MatrixXd Pvq_ref = Eigen::MatrixXd::Zero(dimv_, dimv_);
  ...
  ocp_ref.backwardRiccatiRecursion(dtau_, Pqq_next, Pqv_next, Pvq_next, Pvv_next,  
  ...
  ocp_ref.forwardRiccatiRecursion(dtau_, dq, dv, dq_next_ref, dv_next_ref);
  EXPECT_TRUE(dq_next.isApprox(dq_next_ref));
  EXPECT_TRUE(dv_next.isApprox(dv_next_ref));
  ocp_ref.computeCondensedDirection(dtau_, dq, dv);
  const double primal_step_size_ref = ocp.maxPrimalStepSize();
  const double dual_step_size_ref = ocp.maxPrimalStepSize();
  ...
  const std::pair<double, double> cost_and_violation_ref 
      = ocp_ref.costAndConstraintsViolation(robot_, primal_step_size, t_, 
                                            dtau_, q_, v_, a_, u_, f_, q_next_, 
                                            v_next_, dq, dv, dq_next, dv_next);
  ...
  const double KKT_error_ref
      = ocp_ref.squaredKKTErrorNorm(robot_, t_, dtau_, lmd_, gmm_, q_, v_, a_, 
                                    u_, beta, f_, mu_, lmd_next_, gmm_next_, 
                                    q_next_, v_next_);
...
TEST_F(FixedBaseSplitOCPTest, withActiveContacts) {
  ...
  robot_.setContactStatus(active_constacts);
  ASSERT_FALSE(robot_.has_floating_base());
  ASSERT_EQ(robot_.max_dimf(), 3);
  ...
  cost_ref_->lv(robot_, t_, dtau_, q_, v_, a_, lv);
  cost_ref_->la(robot_, t_, dtau_, q_, v_, a_, la);
  cost_ref_->lf(robot_, t_, dtau_, f_, lf);
  joint_space_constraints_ref_.augmentDualResidual(dtau_, lq, lv, la);
  lq += lmd_next_ - lmd_;
  lv += dtau_ * lmd_next_ + gmm_next_ - gmm_;
  la += dtau_ * gmm_next_;
  ...
  joint_space_constraints_ref_.computeSlackAndDualDirection(dtau_, dq, dv, da, du);
  const double max_primal_step_size = ocp.maxPrimalStepSize();
  const double max_dual_step_size = ocp.maxDualStepSize();
  const double max_primal_step_size_ref 
      = joint_space_constraints_ref_.maxSlackStepSize();
  ...
  const double cost_ref 
      = cost_ref_->l(robot_, t_, dtau_, q_tmp, v_tmp, a_tmp, u_tmp, f_tmp) 
        + joint_space_constraints_ref_.costSlackBarrier(max_primal_step_size);
  const Eigen::VectorXd q_res_tmp 
      = q_tmp - q_next_ + dtau_ * v_tmp - max_primal_step_size * dq_next;
  const Eigen::VectorXd v_res_tmp 
...
TEST_F(FixedBaseSplitOCPTest, KKTError) {
  std::random_device rnd;
  robot_ = Robot(urdf_, contact_frames_, baum_on_velocity_, baum_on_position_);
  std::vector<bool> active_constacts = {rnd()%2==0};
  robot_.setContactStatus(active_constacts);
  ASSERT_FALSE(robot_.has_floating_base());
  ASSERT_EQ(robot_.max_dimf(), 3);
  dimf_ = robot_.max_dimf();
  f_ = Eigen::VectorXd::Random(robot_.max_dimf());
  mu_ = Eigen::VectorXd::Random(robot_.max_dimf());
  cost_.reset();
  cost_ = std::make_unique<manipulator::CostFunction>(robot_);
  ...
  KKT_error_ref 
      += joint_space_constraints_ref_.residualSquaredNrom(dtau_, q_, v_, a_, u_);
