...
TEST_F(FloatingBaseSplitParNMPCTest, initconstraints) {
  ...
  EXPECT_EQ(parnmpc.isFeasible(robot, s), 
            constraints->isFeasible(robot, constraints_data, 
                                    s.a, s.f, s.q, s.v, s.u));
}
...
TEST_F(FloatingBaseSplitParNMPCTest, KKTErrorNorm) {
  ...
  kkt_error_ref += condenser.squaredKKTErrorNorm(dtau);
  kkt_error_ref += constraints->squaredKKTErrorNorm(robot, constraints_data, dtau,
                                                    s.a, s.f, s.q, s.v, s.u);
