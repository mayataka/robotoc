...
protected:
  ...
  std::unique_ptr<quadruped::CostFunction> cost_ref_;
  std::unique_ptr<quadruped::Constraints > constraints_ref_;
...
TEST_F(FloatingBaseSplitOCPTest, isFeasible) {
  SplitOCP ocp_(robot_, cost_, constraints_);
  EXPECT_EQ(ocp_.isFeasible(robot_, q_, v_, a_, u_), 
...
TEST_F(FloatingBaseSplitOCPTest, withoutContacts) {
  ...
  while (!ocp_.isFeasible(robot_, q_, v_, a_, u_)) {
    v_ = Eigen::VectorXd::Random(dimv_);
    u_ = Eigen::VectorXd::Random(dimv_);
  }
  ASSERT_TRUE(ocp_.isFeasible(robot_, q_, v_, a_, u_));
  ocp_.initConstraints(robot_, time_step_, dtau_, q_, v_, a_, u_);
  ocp_.linearizeOCP(robot_, t_, dtau_, lmd_, gmm_, q_, v_, a_, u_, f_, mu_,
  ...
  const double primal_step_size_ref = ocp_ref.maxPrimalStepSize();
  const double dual_step_size_ref = ocp_ref.maxPrimalStepSize();
...
TEST_F(FloatingBaseSplitOCPTest, withActiveContacts) {
  ...
  const Eigen::VectorXd dmu 
      = kmu.head(dimc) + Kmuq.topRows(dimc) * dq + Kmuv.topRows(dimc) * dv;
  ...
  robot_.subtractConfiguration(q_tmp, q_next_, q_res_tmp);
  ...
  f_ref.head(dimf) += max_primal_step_size * df.head(dimf);
  mu_ref.head(dimc) += max_primal_step_size * dmu.head(dimc);
...
TEST_F(FloatingBaseSplitOCPTest, KKTError) {
  ...
  const double KKT_error 
      = ocp_.squaredKKTErrorNorm(robot_, t_, dtau_, lmd_, gmm_, q_, v_, a_, u_, 
                                 beta, f_, mu_, lmd_next_, gmm_next_, q_next_, v_next_);
  Eigen::VectorXd q_res, v_res;
  q_res = Eigen::VectorXd::Zero(dimv_);
  robot_.subtractConfiguration(q_, q_next_, q_res);
