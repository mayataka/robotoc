...
TEST_F(InverseDynamicsTest, fixed_base) {
  ...
  id.augmentInverseDynamics(robot, dtau_, s, kkt_residual);
  ...
  EXPECT_TRUE(kkt_residual.lu.isApprox(lu_ref-dtau_*s.beta));
  lu_ref = kkt_residual.lu;
  lu_ref -= dtau_ * s.beta;
  Eigen::VectorXd lu_condensed = lu_ref + Quu_ref * u_res; 
  id.condenseInverseDynamics(robot, dtau_, s, kkt_matrix, kkt_residual);
...
TEST_F(InverseDynamicsTest, floating_base) {
  ...
  lu_ref = kkt_residual.lu;
  lu_ref -= dtau_ * s.beta;
  lu_ref.head(robot.dim_passive()) += dtau_ * s.mu_active().tail(robot.dim_passive());
  Eigen::VectorXd lu_condensed = lu_ref + Quu_ref * u_res; 
  id.condenseInverseDynamics(robot, dtau_, s, kkt_matrix, kkt_residual);
  EXPECT_TRUE(kkt_residual.lq().isApprox(dtau_*du_dq.transpose()*s.beta+du_dq.transpose()*lu_condensed));
  EXPECT_TRUE(kkt_residual.lv().isApprox(dtau_*du_dv.transpose()*s.beta+du_dv.transpose()*lu_condensed));
  ...
  C.tail(robot.dim_passive()) = dtau_ * (kkt_residual.u_res.head(robot.dim_passive())+s.u.head(robot.dim_passive()));
