...
TEST_F(JointPositionUpperLimitTest, setSlackAndDualFixedBase) {
  ...
  const double l1residual_ref = (dtau_*(s.q-qmax)+slack_ref).lpNorm<1>();
  ...
  const double l2residual_ref 
      = (dtau_*(s.q-qmax)+slack_ref).squaredNorm() + duality_ref.squaredNorm();
  EXPECT_DOUBLE_EQ(l2residual, l2residual_ref);
}


TEST_F(JointPositionUpperLimitTest, setSlackAndDualFixedBase2) {
  JointPositionUpperLimit limit(fixed_base_robot_);
  ConstraintComponentData data(limit.dimc());
  const int dimq = fixed_base_robot_.dimq();
  const int dimv = fixed_base_robot_.dimv();
...
TEST_F(JointPositionUpperLimitTest, setSlackAndDualFloatingBase) {
  ...
  const double l1residual_ref = (dtau_*(s.q.tail(dimc)-qmax)+slack_ref).lpNorm<1>();
  EXPECT_DOUBLE_EQ(l1residual, l1residual_ref);
  ...
  pdipmfunc::ComputeDuality(barrier_, slack_ref, dual_ref, duality_ref);
  const double l2residual_ref 
      = (dtau_*(s.q.tail(dimc)-qmax)+slack_ref).squaredNorm() + duality_ref.squaredNorm();
  EXPECT_DOUBLE_EQ(l2residual, l2residual_ref);
}


TEST_F(JointPositionUpperLimitTest, setSlackAndDualFloatingBase2) {
  JointPositionUpperLimit limit(floating_base_robot_);
  ConstraintComponentData data(limit.dimc());
  const int dimq = floating_base_robot_.dimq();
  const int dimv = floating_base_robot_.dimv();
...
TEST_F(JointPositionUpperLimitTest, condenseSlackAndDualFixedBase) {
  ...
  Eigen::VectorXd residual_ref = dtau_ * (s.q-qmax) + slack_ref;
  Eigen::VectorXd duality_ref = Eigen::VectorXd::Zero(dimq);
  pdipmfunc::ComputeDuality(barrier_, slack_ref, dual_ref, duality_ref);
  Eigen::VectorXd lq_ref = Eigen::VectorXd::Zero(dimq);
  lq_ref.array() 
...
TEST_F(JointPositionUpperLimitTest, condenseSlackAndDualFixedBase2) {
  JointPositionUpperLimit limit(fixed_base_robot_);
  ConstraintComponentData data(limit.dimc());
  const int dimq = fixed_base_robot_.dimq();
  const int dimv = fixed_base_robot_.dimv();
...
TEST_F(JointPositionUpperLimitTest, condenseSlackAndDualFloatingBase) {
  ...
  Eigen::VectorXd residual_ref = dtau_ * (s.q.tail(dimc)-qmax) + slack_ref;
  Eigen::VectorXd duality_ref = Eigen::VectorXd::Zero(dimc);
  pdipmfunc::ComputeDuality(barrier_, slack_ref, dual_ref, duality_ref);
  Eigen::VectorXd lq_ref = Eigen::VectorXd::Zero(dimv);
...
TEST_F(JointPositionUpperLimitTest, condenseSlackAndDualFloatingBase2) {
  JointPositionUpperLimit limit(floating_base_robot_);
  ConstraintComponentData data(limit.dimc());
  const int dimq = floating_base_robot_.dimq();
  const int dimv = floating_base_robot_.dimv();
