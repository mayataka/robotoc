...
TEST_F(FloatingBaseConstraintsTest, isFeasible) {
  ...
  for (int i=0; i<6; ++i) {
    ...
    ASSERT_EQ(data.data[i].dual.size(), robot.dimv()-6);
    ASSERT_EQ(data.data[i].residual.size(), robot.dimv()-6);
    ASSERT_EQ(data.data[i].duality.size(), robot.dimv()-6);
    ASSERT_EQ(data.data[i].dslack.size(), robot.dimv()-6);
    ASSERT_EQ(data.data[i].ddual.size(), robot.dimv()-6);
