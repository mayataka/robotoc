...
TEST_F(FloatingBaseConstraintsTest, isFeasible) {
  ...
  for (int i=0; i<6; ++i) {
    ASSERT_EQ(data[i].slack.size(), robot.dimv()-6);
    ASSERT_EQ(data[i].dual.size(), robot.dimv()-6);
    ASSERT_EQ(data[i].residual.size(), robot.dimv()-6);
    ASSERT_EQ(data[i].duality.size(), robot.dimv()-6);
    ASSERT_EQ(data[i].dslack.size(), robot.dimv()-6);
    ASSERT_EQ(data[i].ddual.size(), robot.dimv()-6);
