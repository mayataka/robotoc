...
TEST_F(JointVelocityLowerLimitTest, setSlackAndDualFixedBase) {
  ...
  const double l1residual_ref = (dtau_*(vmin-s.v)+data_ref.slack).lpNorm<1>();
  ...
  const double l2residual_ref 
      = (dtau_*(vmin-s.v)+data_ref.slack).squaredNorm() + data_ref.duality.squaredNorm();
...
TEST_F(JointVelocityLowerLimitTest, setSlackAndDualFloatingBase) {
  ...
  data_ref.slack = dtau_ * (s.v.tail(dimc)-vmin);
  ...
  const double l1residual = limit.l1NormPrimalResidual(data);
  const double l1residual_ref = (dtau_*(vmin-s.v.tail(dimc))+data_ref.slack).lpNorm<1>();
  EXPECT_DOUBLE_EQ(l1residual, l1residual_ref);
  const double l2residual = limit.squaredNormPrimalAndDualResidual(data);
  Eigen::VectorXd duality_ref = Eigen::VectorXd::Zero(dimc);
  pdipm::ComputeDuality(barrier_, data_ref);
  const double l2residual_ref 
      = (dtau_*(vmin-s.v.tail(dimc))+data_ref.slack).squaredNorm() + data_ref.duality.squaredNorm();
  EXPECT_DOUBLE_EQ(l2residual, l2residual_ref);
}
...
TEST_F(JointVelocityLowerLimitTest, condenseSlackAndDualFixedBase) {
  ...
  data_ref.residual = dtau_ * (vmin-s.v) + data_ref.slack;
...
TEST_F(JointVelocityLowerLimitTest, condenseSlackAndDualFloatingBase) {
  ...
  data_ref.residual = dtau_ * (vmin-s.v.tail(dimc)) + data_ref.slack;
  pdipm::ComputeDuality(barrier_, data_ref);
  Eigen::VectorXd lv_ref = Eigen::VectorXd::Zero(dimv);
