...
TEST_F(JointTorquesLowerLimitTest, setSlackAndDualFixedBase) {
  ...
  const double l1residual_ref = (dtau_*(umin-s.u)+slack_ref).lpNorm<1>();
  ...
  const double l2residual_ref 
      = (dtau_*(umin-s.u)+slack_ref).squaredNorm() + duality_ref.squaredNorm();
  EXPECT_DOUBLE_EQ(l2residual, l2residual_ref);
}


TEST_F(JointTorquesLowerLimitTest, setSlackAndDualFixedBase2) {
  JointTorquesLowerLimit limit(fixed_base_robot_);
  ConstraintComponentData data(limit.dimc());
  const int dimq = fixed_base_robot_.dimq();
  const int dimv = fixed_base_robot_.dimv();
  ...
  limit.augmentDualResidual(fixed_base_robot_, data, dtau_, kkt_residual);
  limit.augmentDualResidual(fixed_base_robot_, data, dtau_, kkt_residual.lu);
  pdipmtest::JointVariablesLowerLimits limit_ref(fixed_base_robot_, -fixed_base_robot_.jointEffortLimit(), barrier_);
...
TEST_F(JointTorquesLowerLimitTest, setSlackAndDualFloatingBase) {
  ...
  slack_ref = dtau_ * (s.u.tail(dimc)-umin);
  Eigen::VectorXd lu_ref = Eigen::VectorXd::Zero(dimv);
  pdipmfunc::SetSlackAndDualPositive(barrier_, slack_ref, dual_ref);
  ...
  const double l1residual_ref = (dtau_*(umin-s.u.tail(dimc))+slack_ref).lpNorm<1>();
  EXPECT_DOUBLE_EQ(l1residual, l1residual_ref);
  ...
  pdipmfunc::ComputeDuality(barrier_, slack_ref, dual_ref, duality_ref);
  const double l2residual_ref 
      = (dtau_*(umin-s.u.tail(dimc))+slack_ref).squaredNorm() + duality_ref.squaredNorm();
  EXPECT_DOUBLE_EQ(l2residual, l2residual_ref);
}


TEST_F(JointTorquesLowerLimitTest, setSlackAndDualFloatingBase2) {
  JointTorquesLowerLimit limit(floating_base_robot_);
  ConstraintComponentData data(limit.dimc());
  const int dimq = floating_base_robot_.dimq();
  const int dimv = floating_base_robot_.dimv();
  ...
  limit.augmentDualResidual(floating_base_robot_, data, dtau_, kkt_residual);
  limit.augmentDualResidual(floating_base_robot_, data, dtau_, kkt_residual.lu);
  pdipmtest::JointVariablesLowerLimits limit_ref(floating_base_robot_, -floating_base_robot_.jointEffortLimit(), barrier_);
  limit_ref.setSlackAndDual(dtau_, s.u);
  Eigen::VectorXd lu_ref = Eigen::VectorXd::Zero(dimv);
  limit_ref.augmentDualResidual(dtau_, lu_ref);
  EXPECT_TRUE(kkt_residual.lu.isApprox(lu_ref));
...
TEST_F(JointTorquesLowerLimitTest, condenseSlackAndDualFixedBase) {
  ...
  Eigen::VectorXd residual_ref = dtau_ * (umin-s.u) + slack_ref;
  Eigen::VectorXd duality_ref = Eigen::VectorXd::Zero(dimq);
  pdipmfunc::ComputeDuality(barrier_, slack_ref, dual_ref, duality_ref);
  Eigen::VectorXd lu_ref = Eigen::VectorXd::Zero(dimq);
  lu_ref.array() 
...
TEST_F(JointTorquesLowerLimitTest, condenseSlackAndDualFixedBase2) {
  JointTorquesLowerLimit limit(fixed_base_robot_);
  ConstraintComponentData data(limit.dimc());
  const int dimq = fixed_base_robot_.dimq();
  const int dimv = fixed_base_robot_.dimv();
...
TEST_F(JointTorquesLowerLimitTest, condenseSlackAndDualFloatingBase) {
  ...
  Eigen::VectorXd residual_ref = dtau_ * (umin-s.u.tail(dimc)) + slack_ref;
  Eigen::VectorXd duality_ref = Eigen::VectorXd::Zero(dimc);
  pdipmfunc::ComputeDuality(barrier_, slack_ref, dual_ref, duality_ref);
  Eigen::VectorXd lu_ref = Eigen::VectorXd::Zero(dimv);
...
TEST_F(JointTorquesLowerLimitTest, condenseSlackAndDualFloatingBase2) {
  JointTorquesLowerLimit limit(floating_base_robot_);
  ConstraintComponentData data(limit.dimc());
  const int dimq = floating_base_robot_.dimq();
  const int dimv = floating_base_robot_.dimv();
