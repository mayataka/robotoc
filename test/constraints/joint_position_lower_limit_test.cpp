...
TEST_F(JointPositionLowerLimitTest, useKinematicsFixedBase) {
  JointPositionLowerLimit limit(fixed_base_robot_); 
  EXPECT_FALSE(limit.useKinematics());
  EXPECT_EQ(limit.kinematicsLevel(), KinematicsLevel::PositionLevel);
}


TEST_F(JointPositionLowerLimitTest, useKinematicsFloatingBase) {
...
TEST_F(JointPositionLowerLimitTest, setSlackAndDualFixedBase) {
  ...
  const double l1residual_ref = (dtau_*(qmin-s.q)+data_ref.slack).lpNorm<1>();
  ...
  const double l2residual_ref 
      = (dtau_*(qmin-s.q)+data_ref.slack).squaredNorm() + data_ref.duality.squaredNorm();
...
TEST_F(JointPositionLowerLimitTest, setSlackAndDualFloatingBase) {
  ...
  data_ref.slack = dtau_ * (s.q.tail(dimc)-qmin);
  ...
  const double l1residual = limit.l1NormPrimalResidual(data);
  const double l1residual_ref = (dtau_*(qmin-s.q.tail(dimc))+data_ref.slack).lpNorm<1>();
  EXPECT_DOUBLE_EQ(l1residual, l1residual_ref);
  const double l2residual = limit.squaredNormPrimalAndDualResidual(data);
  pdipm::ComputeDuality(barrier_, data_ref);
  const double l2residual_ref 
      = (dtau_*(qmin-s.q.tail(dimc))+data_ref.slack).squaredNorm() + data_ref.duality.squaredNorm();
  EXPECT_DOUBLE_EQ(l2residual, l2residual_ref);
}
...
TEST_F(JointPositionLowerLimitTest, condenseSlackAndDualFixedBase) {
  ...
  data_ref.residual = dtau_ * (qmin-s.q) + data_ref.slack;
...
TEST_F(JointPositionLowerLimitTest, condenseSlackAndDualFloatingBase) {
  ...
  data_ref.residual = dtau_ * (qmin-s.q.tail(dimc)) + data_ref.slack;
