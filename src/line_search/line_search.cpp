    ...
    if (i < N) {
      if (ocp.discrete().isTimeStageBeforeImpulse(i)) {
        ocp[i].evalOCP(robots[omp_get_thread_num()], 
                       ...
                       i, ocp.discrete().t(i), ocp.discrete().dt(i), s[i], 
      ...
      ocp.terminal.evalOCP(robots[omp_get_thread_num()], ocp.discrete().t(i), 
                           s[i], kkt_residual_[i]);
