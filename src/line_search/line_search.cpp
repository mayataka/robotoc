  ...
  for (int i=0; i<N_all; ++i) {
    if (i < N) {
      if (ocp.discrete().isTimeStageBeforeImpulse(i)) {
        ocp[i].evaluateOCP(robots[omp_get_thread_num()], 
      ...
      ocp.terminal.evaluateOCP(robots[omp_get_thread_num()], ocp.discrete().t(i), 
                               s[i], kkt_residual_[i]);
      ...
      ocp.impulse[impulse_index].evaluateOCP(robots[omp_get_thread_num()], 
                                             contact_sequence.impulseStatus(impulse_index), 
      ...
      ocp.aux[impulse_index].evaluateOCP(robots[omp_get_thread_num()], 
