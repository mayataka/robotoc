...
void BipedWalkFootStepPlanner::setRaibertGaitPattern(const Eigen::Vector3d& vcom_cmd, 
  ...
  if (swing_time <= 0) {
    throw std::out_of_range(
        "[BipedWalkFootStepPlanner] invalid argument: swing_time must be positive!");
  }
  if (double_support_time < 0) {
    throw std::out_of_range(
        "[BipedWalkFootStepPlanner] invalid argument: double_support_time must be non-negative!");
  }
  if (gain <= 0.0) {
    throw std::out_of_range(
        "[BipedWalkFootStepPlanner] invalid argument: gain must be positive!");
  }
