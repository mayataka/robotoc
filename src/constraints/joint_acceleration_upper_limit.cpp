...
bool JointAccelerationUpperLimit::isFeasible(
  ...
  for (int i=0; i<dimc_; ++i) {
    if (a.tail(dimc_).coeff(i) > amax_.coeff(i)) {
      return false;
    }
  }
  return true;
...
void JointAccelerationUpperLimit::setSlackAndDual(
    ...
    const Eigen::Ref<const Eigen::VectorXd>& u) const {
  assert(dtau > 0);
  data.slack = dtau * (amax_-a.tail(dimc_));
...
void JointAccelerationUpperLimit::condenseSlackAndDual(
  ...
  data.residual = dtau * (a.tail(dimc_)-amax_) + data.slack;
