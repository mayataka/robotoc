...
ImpulseFrictionCone::ImpulseFrictionCone(const Robot& robot, const std::vector<double>& mu)
  ...
  if (mu.size() != robot.maxNumContacts()) {
    throw std::out_of_range(
        "[ImpulseFrictionCone] invalid argument: mu.size() must be" + std::to_string(robot.maxNumContacts()) + "!");
  }
  for (int i=0; i<robot.maxNumContacts(); ++i) {
    if (mu[i] <= 0) {
      throw std::out_of_range(
          "[ImpulseFrictionCone] invalid argument: mu[" + std::to_string(i) + "] must be positive!");
    }
  }
  for (int i=0; i<robot.maxNumContacts(); ++i) {
    cone_[i] <<  0,  0, -1, 
...
void ImpulseFrictionCone::setFrictionCoefficient(const std::vector<double>& mu) {
  if (mu.size() != max_num_contacts_) {
    throw std::out_of_range(
        "[ImpulseFrictionCone] invalid argument: mu.size() must be" + std::to_string(max_num_contacts_) + "!");
  }
