...
QuadrupedSimulator::QuadrupedSimulator(
...
void QuadrupedSimulator::run(OCPSolver& ocp_solver, 
                             std::shared_ptr<MPCCallBackBase>& callback, 
  ...
  ocp_solver.getControlInput(u);
  q_raisim = helper::pino2rai_q(q);
  v_raisim = helper::pino2rai_v(robot, q, v);
  raisim_robot->setState(q_raisim, v_raisim);
  raisim_robot->setGeneralizedForce(helper::pino2rai_u(u));
  ...
  callback.init(simulation_start_time_in_sec, q, v, ocp_solver);
  for (double t=simulation_start_time_in_sec; t<simulation_time_in_sec; t+=sampling_period_in_sec) {
    ...
    q = helper::rai2pino_q(q_raisim);
    v = helper::rai2pino_v(robot, q, v_raisim);
    callback.callback(t, q, v, ocp_solver);
    ocp_solver.computeKKTResidual(t, q, v);
    data_saver_.save(q, v, u, ocp_solver.KKTError());
...
void QuadrupedSimulator::SetupRaisimOgre(raisim::World& world) {
  auto vis = raisim::OgreVis::get();
  vis->setWorld(&world);
  vis->setSetUpCallback(QuadrupedSimulator<OCPSolverType>::RaiSimOgreCallback);
...
void QuadrupedSimulator::RaiSimOgreCallback() {
  auto vis = raisim::OgreVis::get();
  /// light
