...
void TrottingSwingFootRef::update_x3d_ref(const ContactStatus& contact_status, 
  ...
  if (std::abs(xdiff) < eps) {
    ...
    x3d_ref.coeffRef(0) = contact_status.contactPoint(x_ref_foot_contact_index_).coeff(0);
    x3d_ref.coeffRef(0) += 0.25 * step_length_;
  }
  else {
    x3d_ref.coeffRef(0) = contact_status.contactPoint(x_ref_foot_contact_index_).coeff(0);
  }
  x3d_ref.coeffRef(1) = contact_status.contactPoint(y_ref_foot_contact_index_).coeff(1);
