...
inline ContactSequence::ContactSequence(const Robot& robot, 
  ...
  if (reserved_num_discrete_events < 0) {
    throw std::out_of_range(
        "[ContactSequence] invalid argument: reserved_num_discrete_events must be non-negative!");
  }
  clear_all();
...
inline void ContactSequence::push_back(const DiscreteEvent& discrete_event, 
  ...
  if (numImpulseEvents() > 0 || numLiftEvents() > 0) {
    if (event_time <= event_time_.back()) {
      throw std::runtime_error(
          "[ContactSequence] The input event_time " + std::to_string(event_time) 
...
inline void ContactSequence::setImpulseTime(const int impulse_index, 
  ...
  if (numImpulseEvents() <= 0) {
    throw std::runtime_error(
        "[ContactSequence] numImpulseEvents() must be positive when calling this method!");
  }
  if (impulse_index < 0) {
    throw std::runtime_error(
        "[ContactSequence] impulse_index must be non-negative!");
  }
  if (impulse_index >= numImpulseEvents()) {
    throw std::runtime_error(
        "[ContactSequence] The input impulse_index " + std::to_string(impulse_index) 
...
inline void ContactSequence::setLiftTime(const int lift_index, 
  ...
  if (numLiftEvents() <= 0) {
    throw std::runtime_error(
        "[ContactSequence] numLiftEvents() must be positive when calling this method!");
  }
  if (lift_index < 0) {
    throw std::runtime_error(
        "[ContactSequence] lift_index must be non-negative!");
  }
  if (lift_index >= numLiftEvents()) {
    throw std::runtime_error(
        "[ContactSequence] The input lift_index " + std::to_string(lift_index) 
...
inline void ContactSequence::setContactPlacements(
  ...
  if (contact_phase >= numContactPhases()) {
    throw std::runtime_error(
        "[ContactSequence] The input contact_phase " + std::to_string(contact_phase) 
